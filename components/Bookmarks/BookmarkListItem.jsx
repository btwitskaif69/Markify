...
function BookmarkListItem({
  ...
  return (
    <Card
      className={`p-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 transition-all w-full min-w-0 overflow-hidden ${isSelected ? "ring-2 ring-primary bg-primary/5" : ""
        } ${isSelectionMode ? "cursor-pointer hover:bg-muted/50" : ""}`}
      onClick={handleRowClick}
    >
      {/* Left Section: Info */}
      <div className="flex items-start sm:items-center gap-4 flex-1 min-w-0 overflow-hidden">
        {/* Selection Checkbox */}
        {isSelectionMode && (
          <Checkbox
            checked={isSelected}
