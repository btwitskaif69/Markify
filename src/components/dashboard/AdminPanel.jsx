...
export default function AdminPanel() {
  ...
  useEffect(() => {
    const fetchPendingCount = async () => {
      try {
        const res = await authFetch(`${API_URL}/reviews/pending/count`);
        if (res.ok) {
          const data = await res.json();
  ...
  useEffect(() => {
    if (isAdmin && showReviewManager) {
      fetchPendingReviews();
    }
  }, [isAdmin, showReviewManager]);
  ...
  const handleApproveReview = async (reviewId) => {
    try {
      const res = await authFetch(`${API_URL}/reviews/${reviewId}/approve`, {
        method: "PATCH",
      });
  ...
  const handleRejectReview = async (reviewId) => {
    ...
    try {
      const res = await authFetch(`${API_URL}/reviews/${reviewId}/reject`, {
        method: "PATCH",
      });
  ...
  const renderOverview = () => (
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl border bg-card p-6 shadow-sm hover:shadow-md transition-all cursor-pointer group relative overflow-hidden opacity-50">
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
  ...
  const renderReviewManager = () => (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button
          variant="outline"
          onClick={() => setShowReviewManager(false)}
        >
          Close Manager
        </Button>
      </div>
      ...
      <SidebarInset className="flex flex-col min-h-screen bg-background">
        ...
        <main className="flex-1 p-6">
          <section className="max-w-6xl mx-auto space-y-6">
            {showBlogManager ? renderBlogManager() : showReviewManager ? renderReviewManager() : renderOverview()}
          </section>
        </main>
      </SidebarInset>
    </SidebarProvider >
